...
#define BRIGHTNESS_NUM	2	// number of power levels
...
#define B_DELAY		6000	// toggle brightness
#define W_DELAY		9000	// toggle mode of White channel
#define W_FAV_DELAY	12000
#define T_FAV_DELAY	16000
#define CLEAR_DELAY 19000
...
void read_data(){
	...
	wmode = eeprom_read_byte( &e_wmode );
...
void store_data(){
	...
	eeprom_write_byte(&e_fav_on,   fav_on);//Запись	
	
	eeprom_write_byte( &e_power, power );
	
	eeprom_write_byte( &e_brightness, brightness );
	
	eeprom_write_byte( &e_wmode, wmode );
}

void init_io(){
	LED_DDR = (1<<LED_1) | (1<<LED_2) | (1<<LED_3) | (1<<LED_4) | (0<<BUTTON_1) | (0<<BUTTON_2);	
	LED_PORT = (1<<LED_1) | (1<<LED_2) | (1<<LED_3) | (1<<LED_4)  | (1<<BUTTON_1) | (1<<BUTTON_2);	
}
...
unsigned char wake(){
	...
	if ( ( (st &  (1<<BUTTON_1))  == (0<<BUTTON_1) ) ) {
		power=ACTIVE;
		//_interrupts_off();
		LED_PORT= (1<<LED_1) | (1<<LED_2) |(1<<BUTTON_1) | (1<<BUTTON_2) | (1<<LED_4) ;
	...
	while(1){	// <><><><><><><><><><><><><><><><><><><><><><><><><><><>
		if ( power==AWAKEN ){
			...
			* */
			...
			make_color(16);			
		} else if (stat==17) {	// toggle White channel
			make_color(17);			
		} else{	
			make_color(0);
		} 
...
unsigned char process_button( unsigned int hold, unsigned int state){
	...
	}  else if ( hold > W_DELAY ){	// toggle White mode
	//	if (fav_on==0){
			stat=17;
	//		if (fav_on==0){
				if ( state == NOT_PRESSED ){
...
unsigned char basic_pulse( unsigned char r, unsigned char g, unsigned char b, unsigned char rr, unsigned char gg, unsigned char bb,  unsigned int tt){
	...
	for (i=1; i<=ALL_STEPS; i++){
		if ( l_make_light( (i&r)|rr, (i&g)|gg, (i&b)|bb , tt)==1 || stat!=0)  return 1;			 	
	}
...
unsigned char make_pulse( unsigned char c ){
	switch (c){
		...
		case 7:
			...
			break;
		case 8:
			return basic_pulse( 255, 63, 0, 255,0,0, PULSW_SPEED );	//pink
			break;
		case 9:
			return basic_pulse( 255, 0, 63, 25,0,0, PULSW_SPEED );	//white
			break;
		}
	return 0;
}
...
unsigned char make_color( unsigned char c ){
		...
		switch (c){
		case 10:
			return make_light( 0, LOW_LEVEL, 0 );	//
		case 11:
			return make_light( LOW_LEVEL, 0, 0 );	//
		case 12:
			return make_light( LOW_LEVEL, LOW_LEVEL, LOW_LEVEL );	//
		case 13:
			return make_light( 0, 0, LOW_LEVEL );	//	
		case 14:
			return make_light( LOW_LEVEL, LOW_LEVEL, 0 );	//		
		}
		
		switch (c){
		case 15:
			return make_light( LOW_LEVEL, LOW_LEVEL, LOW_LEVEL );	//violet
		case 16:
			return make_light( LOW_LEVEL, 0, LOW_LEVEL );	//pink
		case 17:
			return make_light( 0, LOW_LEVEL, LOW_LEVEL );	//aqua
			break;
		}		
