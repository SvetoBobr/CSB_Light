// t85	attiny85	115000 prog
// board	MMCU		programm_after_make
//38400

// прошивка с избранным
// с софтварным питанием
// multiple brightness settings added

#define F_CPU 8000000	// need faster MCU ((8M instead of 1M))
...
// values for PWM -----------------------------------------------------
#define BRIGHTNESS_NUM	4	// number of power levels
unsigned char brightness;	// set power level;
#define ALL_STEPS	90					//20

#define LIGHT_1		light_r[brightness] //(ALL_STEPS-0)
#define LIGHT_2		light_g[brightness] //(ALL_STEPS-0)
#define LIGHT_3		light_b[brightness] //(ALL_STEPS-0)
#define HALF_LIGHT	half_light[brightness] // 5

// brightness
unsigned char light_delay[BRIGHTNESS_NUM]={1,1,1,1};
unsigned char light_r[BRIGHTNESS_NUM]={90,45,15,9};
#define light_g light_r
#define light_b light_r
unsigned char half_light[BRIGHTNESS_NUM]={27,13,4,2};
...
#define INC_DELAY	100
#define S_DELAY		1000
#define HALT_DELAY  3000	// more often used function then next
#define B_DELAY		7000	// toggle brightness
#define W_FAV_DELAY	10000
#define T_FAV_DELAY	13000
...
EEMEM unsigned char	e_brightness;
...
void read_data(){
	...
	brightness = eeprom_read_byte(&e_brightness);//Чтение
...
void store_data(){
	...
	eeprom_write_byte(&e_brightness,   e_brightness);//Запись
...
unsigned char wake(){
	...
	if ( ( (st &  (1<<BUTTON_1))  == (0<<BUTTON_1) ) ) {
		power=ACTIVE;
		//_interrupts_off();
		LED_PORT= (1<<LED_1) | (1<<LED_2) |(1<<BUTTON_1) | (1<<BUTTON_2);
		...
		power=TO_SLEEP;
		//
		LED_PORT= (1<<LED_3) | (1<<BUTTON_1) | (1<<BUTTON_2);
		return 0;
	}
}
	...
	while(1){	// <><><><><><><><><><><><><><><><><><><><><><><><><><><>
		if ( power==AWAKEN ){
			...
			* */
			...
			make_color(15);			
		} else if (stat==16) {	// toggle brightness
			make_color(16);			
		} else{	
			make_color(0);
		} 
...
unsigned char process_button( unsigned int hold, unsigned int state){
	...
	}  else if ( hold > B_DELAY ){	// toggle brightness
	//	if (fav_on==0){
			stat=16;
	//		if (fav_on==0){
				if ( state == NOT_PRESSED ){
...
//unsigned int pulsw_speed[4]={200,400,800,1600};
//#define PULSW_SPEED	pulsw_speed[brightness-1] //200
...
unsigned char make_pulse( unsigned char c ){
	switch (c){
		...
		case 7:
			...
			break;
		case 8:
			return basic_pulse( 255, HALF_LIGHT, 0, 255,0,0, PULSW_SPEED );	//pink
			break;
		case 9:
			return basic_pulse( 255, 0, HALF_LIGHT, 255,0,0, PULSW_SPEED );	//white
			break;
		}
	return 0;
}
...
void make_grad( unsigned int tt ){
	unsigned int i;
	for (i=0; i<=LIGHT_1; i++){
		if ( l_make_light(LIGHT_1,i,0, tt )==1 ||stat!=0)  return;			 	// red >> yellow
	}
	for (i=1; i<=LIGHT_1; i++){
		if ( l_make_light(LIGHT_1-i, LIGHT_1,0, tt )==1 || stat!=0) return; 	// yellow >> green
	}
	for (i=1; i<=LIGHT_1; i++){
		if ( l_make_light(0, LIGHT_1, i, tt )==1 || stat!=0) return;			// green >> blue
	}
	for (i=1; i<=LIGHT_1; i++){
		if ( l_make_light(0, LIGHT_1-i, LIGHT_1, tt )==1 || stat!=0) return;  // blue >> dark blue
	}
	for (i=1; i<=LIGHT_1; i++){	
		if ( l_make_light(i,0,LIGHT_1, tt )==1 || stat!=0) return;				// dark blue >> violet
	}
	for (i=1; i<=LIGHT_1; i++){				
		if ( l_make_light(LIGHT_1,0,LIGHT_1-i, tt )==1 || stat!=0) return;			// violet >> red
	}
}
...
void make_saw_rainbow(){
	unsigned char i;
	for(i=0; i<LIGHT_1; i++){
		if ( l_make_light( LIGHT_1, LIGHT_1-i, LIGHT_1-i, 2 ) == 1 || stat!=0 ) return;	// white to red
	}
	for(i=0; i<ALL_STEPS; i++){
		if ( l_make_light( LIGHT_1, i, 0, 2 ) == 1 || stat!=0 ) return;	// red to yellow
	}	
	for(i=0; i<LIGHT_1; i++){
		if ( l_make_light( LIGHT_1-i, LIGHT_1, 0, 2 ) == 1 || stat!=0 ) return ;	// yellow to green
	}	
	for(i=0; i<LIGHT_1; i++){
		if ( l_make_light( 0, LIGHT_1, i, 2 ) == 1 || stat!=0 ) return ;	// green to green-blue
	}	
	for(i=0; i<LIGHT_1; i++){
		if ( l_make_light( 0, LIGHT_1-i, LIGHT_1, 2 ) == 1 || stat!=0 ) return ;	// green-blue to blue
	}
	for(i=0; i<LIGHT_1; i++){
		if ( l_make_light( 0, 0, LIGHT_1-i, 2 ) == 1 || stat!=0 ) return ;	// blue to dark
	}
...
unsigned char make_light_color( unsigned char c ){
		...
		case 1: 
			l_make_light(0,0,0,10);
			return make_light( 0, 0, 0 );	//black
		case 2:
			l_make_light(0,0,0,10);
			return make_light( 3, 0, 0 );	//orange
		case 3:
			l_make_light(0,0,0,10);
			return make_light( 3, 3, 0 );	//yellow
		}
		
		switch (c){
		case 4:
			l_make_light(0,0,0,10);
			return make_light( 0, 3, 0 );	//green
		case 5:
			l_make_light(0,0,0,10);
			return make_light( 0, 3, 3 );	//blue
		case 6:
			l_make_light(0,0,0,10);
			return make_light( 0, 0, 3 );	//dark blue
		}
		
		switch (c){
		case 7:
			l_make_light(0,0,0,10);
			return make_light( 3, 0, 3 );	//violet
		case 8:
			l_make_light(0,0,0,10);
			return make_light( 3, 1, 1 );	//pink
		case 9:
			return make_light( 3, 3, 3 );	//white
		}
	return 0;
}
...
unsigned char l_make_light( unsigned char r, unsigned char g, unsigned char b, unsigned int tt ){
	unsigned int i;
	for(i=0; i<tt*light_delay[brightness] ; i++){
...
unsigned char make_color( unsigned char c ){
		...
		case 8:
			return make_light( LIGHT_1, HALF_LIGHT, HALF_LIGHT );	//pink
		case 9:
...
unsigned char long_color( unsigned int time, unsigned int c ){
	unsigned int i;
	for (i=0 ; i<time*light_delay[brightness] ; i++){
