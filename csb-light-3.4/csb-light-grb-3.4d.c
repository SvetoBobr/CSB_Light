...
// brightness
 float light_delay[BRIGHTNESS_NUM]={1, 1.2, 1.6, 2};
...
unsigned char l_make_light( unsigned char r, unsigned char g, unsigned char b, float tt );
unsigned char make_color( unsigned char c ); // (7 simple colors)

unsigned char long_color( float time, unsigned int c ); // (7 simple colors)
...
unsigned char long_color( float time, unsigned int c ){
	unsigned int i;
