...
#define HALT_DELAY  3000	// more oten function then next
...
unsigned char make_pulse( unsigned char c ){
	switch (c){
		case 1: 
			...
			break;
		case 2:
			return basic_pulse( 255, 0, 0,0,0,255, PULSW_SPEED );	//yellow
			break;
		case 3:
			return basic_pulse( 0, 255, 0,0,0,255, PULSW_SPEED );	//green
			break;
		}
		...
		case 4:
			...
			break;
		case 5:
			return basic_pulse( 0, 0, 255,7,25,0, PULSW_SPEED );	//blue
			break;
		case 6:
		...
		switch (c){
		case 7:
			return basic_pulse( 0, 255, 0,7,0,255, PULSW_SPEED );	//white
			break;
		case 8:
			return basic_pulse( 255, 255, 255, 255,0,0, PULSW_SPEED );	//red-white
			break;
		case 9:
			return basic_pulse( 255, 0, 0, 0,7,255, PULSW_SPEED );	//lightblue-violet
			break;
		}
	return 0;
}
...
void make_complex_strob(unsigned char m){
	switch (m){
		case 1: 
			...
			if ( make_strob(45,45, 7)==1 || stat!=0 ) return;	//
			return;
		case 2:
			...
			return;
		case 3:
			if ( make_strob(45,45, 3)==1 || stat!=0 ) return;	//
			if ( make_strob(45,45, 7)==1 || stat!=0 ) return;	//
			return;
		case 4:
...
void make_mix( unsigned char m ){
	switch (m){
		case 1: 
			...
			if ( long_color(60, 7)==1 || stat!=0 ) return;	//
			return;
		case 2:
			...
			return;
		case 3:
			if ( long_color(60, 3)==1 || stat!=0 ) return;	//
			if ( long_color(60, 7)==1 || stat!=0 ) return;	//
			return;
		case 4:
			if ( long_color(60, 1)==1 || stat!=0 ) return;	//
		...
		case 5:
			if ( long_color(60, 1)==1 || stat!=0 ) return;	//
			if ( long_color(60, 9)==1 || stat!=0 ) return;	//
			return;
	}
	...
	switch (m){
		case 6:
			...
			if ( long_color(45, 8)==1 || stat!=0 ) return;	//
			if ( long_color(45, 5)==1 || stat!=0 ) return;	//
			return;
		case 7:
			if ( long_color(45, 0)==1 || stat!=0 ) return;	//
		...
		case 8:
			...
			return;
		case 9:
			if ( long_color(60, 0)==1 || stat!=0 ) return;	//
			if ( long_color(60, 1)==1 || stat!=0 ) return;	//
			if ( long_color(60, 0)==1 || stat!=0 ) return;	//
			if ( long_color(60, 9)==1 || stat!=0 ) return;	//
			return;
...
unsigned char make_color( unsigned char c ){
		...
		case 5:
			return make_light( 0, LIGHT_2, LIGHT_3 );	//Light blue
		case 6:
			return make_light( 0, 0, LIGHT_3 );	//blue
		}
		...
		case 8:
			return make_light( 0, LIGHT_2, HALF_LIGHT );	//seacolor
		case 9:
